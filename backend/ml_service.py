...
svc = pickle.load(open(os.path.join(BASE_DIR, "ml_models", "svc.pkl"), "rb"))
...
def get_all_symptoms():
    return sorted(symptoms_dict.keys())
...
def get_disease_details(dis):
    ...
    pre = [p for p in pre.values.flatten() if pd.notna(p)]

    def parse_stringified_list(series):
        """Some CSV columns store values like "['A', 'B', 'C']" as one string. Parse them properly."""
        result = []
        for val in series:
            try:
    ...
    med = parse_stringified_list(medications_df[medications_df['Disease'] == dis]['Medication'])
    die = parse_stringified_list(diets_df[diets_df['Disease'] == dis]['Diet'])
